...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if has_many_gpus():
            if x.abs().mean() < 1e-4:
                r1 = torch.zeros(
                    (1, self.n_rkhs, 1, 1), device=x.device, dtype=x.dtype
                ).detach()
                r5 = torch.zeros(
                    (1, self.n_rkhs, 5, 5), device=x.device, dtype=x.dtype
                ).detach()
                r7 = torch.zeros(
                    (1, self.n_rkhs, 7, 7), device=x.device, dtype=x.dtype
                ).detach()
                return r1, r5, r7
...
class Evaluator(nn.Module):
    def __init__(self, n_classes, ftr_1=None, dim_1=None):
        ...
        self.block_glb_mlp = MLPClassifier(
            self.dim_1, self.n_classes, n_hidden=1024, p=0.2
        )
        self.block_glb_lin = MLPClassifier(
            self.dim_1, self.n_classes, n_hidden=None, p=0.0
        )
...
class Model(nn.Module):
    ...
    def forward(self, x1, x2, class_only=False):
        ...
        loss_1t5, loss_1t7, loss_5t5, lgt_reg = self.g2l_loss(
...
class Conv3x3(nn.Module):
    def __init__(
        self, n_in, n_out, n_kern, n_stride, n_pad, use_bn=True, pad_mode="constant"
    ):
    ...
    def __init__(self, n_input, n_classes, n_hidden=512, p=0.1):
        ...
        if n_hidden is None:
            ...
            self.block_forward = nn.Sequential(
                Flatten(), nn.Dropout(p=p), nn.Linear(n_input, n_classes, bias=True)
            )
        else:
...
class FakeRKHSConvNet(nn.Module):
    def __init__(self, n_input, n_output, use_bn=False):
        ...
        self.conv1 = nn.Conv2d(
            n_input, n_output, kernel_size=1, stride=1, padding=0, bias=False
        )
        self.relu1 = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(
            n_output, n_output, kernel_size=1, stride=1, padding=0, bias=False
        )
        ...
        self.shortcut = nn.Conv2d(
            n_input, n_output, kernel_size=1, stride=1, padding=0, bias=True
        )
