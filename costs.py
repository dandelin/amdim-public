...
def loss_xent(logits, labels, ignore_index=-1):
    ...
    """
    xent = F.cross_entropy(tanh_clip(logits, 10.0), labels, ignore_index=ignore_index)
    lgt_reg = 1e-3 * (logits ** 2.0).mean()
    return xent + lgt_reg


class NCE_MI_MULTI(nn.Module):
    def __init__(self, tclip=20.0):
    ...
    def _model_scores(self, r_src, r_trg, mask_mat):
        ...
        """
        neg_sumexp = (mask_neg * torch.exp(neg_scores - neg_maxes)).sum(
            dim=1, keepdim=True
        )
        """
    ...
    def _loss_g2l(self, r_src, r_trg, mask_mat):
        ...
        nce_scores, raw_scores, lgt_reg = self._model_scores(r_src, r_trg, mask_mat)
    ...
    def forward(
        ...
        if mode == "train":
            ...
            nce_scores, raw_scores, lgt_reg = self._model_scores(
                r1_src_1, r7_trg_2[0], mask_mat
            )
...
class LossMultiNCE(nn.Module):
    ...
    def forward(self, r1_x1, r5_x1, r7_x1, r1_x2, r5_x2, r7_x2):
        ...
        if n_gpus >= 4:
            assert n_batch % (n_gpus - 1) == 0, "n_batch: {}, n_gpus: {}".format(
                n_batch, n_gpus
            )
        ...
        loss_1t5, loss_1t7, loss_5t5, lgt_reg = self.nce_func(
            r1_src_1,
            r5_src_1,
            r1_src_2,
            r5_src_2,
